...
export default function () {
  ...
  const getDetails = async () => {
    let filter = `product.name ~ '${search.name}' && product.price >= ${search.minPrice} && product.price <= ${search.maxPrice} `
    for (let category of search.categories) {
      filter += `&& product.category.name ~ '${category}' `
    }
    for (let tag of search.tags) {
      filter += `&& product.tags.name ~ '${tag}' `
    }
    for (let size of search.sizes) {
      filter += `&& product.sizes.name ~ '${size}' `
    }
    ...
    setProducts({
      ...
      items: res.items.map((_favourite) => _favourite['@expand'].product)
    })
            ...
            <div className="col-lg-9">
              {products.items.length ? (
                ...
                <div className="d-flex flex-column align-items-center py-3">
                  <i
                    className="fa fa-heart-o text-muted mb-2"
                    aria-hidden="true"
                    style="font-size: 4rem"
                  ></i>
                  <h3 className="text-muted">No favourited item!</h3>
                </div>
              )}
            </div>
          </div>
